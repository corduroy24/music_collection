...
void MyTunes::executeCMDFOLLOW(Command cmd){
	...
	if(cmd.hasToken("-u") && cmd.hasToken("-p") && cmd.hasToken("-f")) //maybe check for string lengths
		executeFollowUserPlaylist(cmd);

	else if(cmd.hasToken("-u") && cmd.hasToken("-p") && cmd.hasToken("-f") && cmd.hasToken("stop"))
		executeStopFollowUserPlaylist(cmd);

  else view.printOutput("EXECUTING: " + cmd.getCommandString());
}

void MyTunes::executeFollowUserPlaylist(Command cmd){
	//follow -u userid -p playlist_name -f userid2
	//follow -u ajones -p brass_monkey_set1 -f cindy

	string user_id = cmd.getToken("-u");
	string playlistName = cmd.getToken("-p");
	string user_id_2 = cmd.getToken("-f");

	if(user_id_2.compare("stop") == 0) {
		executeStopFollowUserPlaylist(cmd);
		return;
	}
		...
		User* userToFollow = users.findByID(user_id_2);
		if(userToFollow == NULL) return;

		if(user == userToFollow){
			cout << "\nSORRY YOU CANT FOLLOWt YOUR OWN PLAYLIST \n" << endl;
			return;
		}

		Playlist * playlistToFollow = userToFollow->findPlaylist(playlistName);
		if(playlistToFollow == NULL) {
			cout << "\nSORRY THIS IS A NON-EXISTING LIST \n" << endl;
			return;
		}

		Playlist * playlistFollowee = user->findPlaylist(playlistName);

		if(playlistFollowee == NULL){
			playlistFollowee = new Playlist(playlistName);
			user->addPlaylist(*playlistFollowee);
		}

		playlistFollowee->startFollowing(*playlistToFollow);

		view.printOutput("EXECUTING: FOLLOW USER PLAYLIST " + cmd.getCommandString());
	}
}



void MyTunes::executeStopFollowUserPlaylist(Command cmd){
	//follow -u userid -p playlist_name -f stop
	//follow -u ajones -p brass_monkey_set1 -f stop
	string user_id = cmd.getToken("-u");
  string playlistName = cmd.getToken("-p");
	...
	Playlist * playlistToStopFollowing = user->findPlaylist(playlistName);
	if(playlistToStopFollowing == NULL) {
		cout << "\nSORRY THIS IS A NON-EXISTING LIST \n" << endl;
		return;
	}

	playlistToStopFollowing->stopFollowing();

	view.printOutput("EXECUTING: STOP FOLLOW USER PLAYLIST " + cmd.getCommandString());
}
//CMD SHOW COMMANDS
